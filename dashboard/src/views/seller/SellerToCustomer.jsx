...
const SellerToCustomer = () => {
    ...
    useEffect(() => {
        if (successMessage) {
            console.log("Latest message sent by the seller:", messages[messages.length - 1]); // Logs the latest message sent by the seller
            ...
            <div className='w-full bg-[#2c3e50] px-4 py-4 rounded-md h-[calc(100vh-140px)]'>
                <div className='flex w-full h-full relative'>
                    ...
                    <div className={`w-[280px] h-full absolute z-10 ${show ? '-left-[16px]' : '-left-[336px]'} md:left-0 md:relative transition-all`}>
                        <div className='w-full h-[calc(100vh-177px)] bg-[#34495e] md:bg-transparent overflow-y-auto'>
                            ...
                            {customers.map((c, i) => (
                                <Link key={i} to={`/seller/dashboard/chat-customer/${c.fdId}`} className={`h-[60px] flex justify-start gap-2 items-center text-white px-2 py-2 rounded-md cursor-pointer bg-[#5d6d7e]`}>
                    ...
                    <div className='w-full md:w-[calc(100%-200px)] md:pl-4'>
                        <div className='flex justify-between items-center'>
                            ...
                            <div onClick={() => setShow(!show)} className='w-[35px] flex md:hidden h-[35px] rounded-sm bg-[#1abc9c] shadow-lg hover:shadow-[#1abc9c]/50 justify-center cursor-pointer items-center text-white'>
                                <span><FaList /> </span>
                            </div>
                        </div>
                                    ...
                                    m.senderId === customerId ? (
                                        <div key={i} ref={scrollRef} className='w-full flex justify-start items-center'>
                                            <div className='flex justify-start items-start gap-2 md:px-3 py-2 max-w-full lg:max-w-[85%]'>
                                                ...
                                                <div className='flex justify-center items-start flex-col w-full bg-[#1abc9c] shadow-lg shadow-[#1abc9c]/50 text-white py-1 px-2 rounded-sm'>
                                                    <span>{m.message}</span>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div key={i} ref={scrollRef} className='w-full flex justify-end items-center'>
                                            <div className='flex justify-start items-start gap-2 md:px-3 py-2 max-w-full lg:max-w-[85%]'>
                                                <div className='flex justify-center items-start flex-col w-full bg-[#e74c3c] shadow-lg shadow-[#e74c3c]/50 text-white py-1 px-2 rounded-sm'>
                                                    <span>{m.message}</span>
                                                </div>
                                                <div>
                        ...
                        {customerId && (
                            <form onSubmit={send} className='flex gap-3'>
                                <input
                                    ...
                                    className='w-full flex justify-between px-2 border border-slate-700 items-center py-[5px] focus:border-[#1abc9c] rounded-md outline-none bg-transparent text-white'
                                    type="text"
                                    placeholder='Input Your Message'
                                />
                                <button className='shadow-lg bg-[#3498db] hover:shadow-[#3498db]/50 text-semibold w-[75px] h-[35px] rounded-md text-white flex justify-center items-center'>Send</button>
                            </form>
                        )}
                    </div>
                </div>
